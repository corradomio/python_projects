...
def prepare_autoregressive_train_data(df, dict_hp):
    ...
    train, test, train_X, train_Y, test_X, test_Y = tth.createTrainTestSets(
        df, train_ratio, targetFeature,
        ignoreInputFeatures + categoricalFeatures
    )

    assert targetFeature not in train_X.columns
    assert targetFeature not in test_X.columns
...
def run_regression(train_X, train_Y, test_X, test_Y, list_regression_models, printLog):
    ...
    bestModelName, bestModel, bestWape, bestR2, bestDfPred = \
        tth.findBestModel(list_regression_results)
...
def run_training(df, dict_hp, list_regression_models, printLog):
    ...
    list_regression_results, bestModelName, bestModel, bestWape, bestR2, bestDfPred \
        = run_regression(train_X, train_Y, test_X, test_Y, list_regression_models, printLog)

    return test, bestDfPred, bestModelName, bestWape, bestR2, list_regression_results, list_ohmodels_catFtr, bestModel


def run_prediction(dict_hp, dfp, list_ohmodels_catFtr, bestModel, printLog):
    ...
    # Run prediction
    dfp = pdh.runPredictionForAllNewPeriods_new(dfp, bestModel, targetWeekLag, targetDayLag, inputsWeekLag, inputsDayLag, dowCol, targetFeature, inputFeaturesForAutoRegression, ignoreInputFeatures, printLog)

    return dfp, pred_period


def run_training_prediction(dict_hp, df, dfp, list_regression_models):
    log = logging.getLogger("ipredict.ipt17.run_train_prediction")
    log.info("run_training_prediction")

    # Prepare data and Run training
    test, bestDfPred, bestModelName, bestWape, bestR2, list_regression_results, list_ohmodels_catFtr, bestModel = run_training(df, dict_hp, list_regression_models, log)

    # Prepare data and Run prediction
    dfp, pred_period = run_prediction(dict_hp, dfp, list_ohmodels_catFtr, bestModel, log)
...
def run_training_by_area(df_train, dict_hp, list_regression_models):
    log = logging.getLogger("ipredict.ipt17.run_training_by_area")
    log.info("run_training_by_area")
    ...
    for curArea in dict_of_regions_train:

        log.info(f"... {curArea}")

        df = dict_of_regions_train[curArea].copy()
        ...
        try:
            test, bestDfPred, bestModelName, bestWape, bestR2, list_regression_results, list_ohmodels_catFtr, bestModel \
                = run_training(df, dict_hp, list_regression_models, log)

            log.info(f"... best_model: {bestModelName} (wape={bestWape:.2}, r2={bestR2:.2})")
        except Exception as e:
            # print(str(curArea) + ' \t is skipping, Something went wrong when trying to do training')
            log.error(f"[{curArea}] prediction failed:")
        ...
        dict_ohmodels_catFtr_areas[curArea] = list_ohmodels_catFtr
        dict_best_model_areas[curArea] = copy.deepcopy(bestModel)
    ...
    log.info("end [run_training_by_area]")
...
def run_prediction_by_area(df_pred, dict_hp, dict_ohmodels_catFtr_areas, dict_best_model_areas):
    log = logging.getLogger("ipredict.ipt17.run_training_by_area")
    log.info("run_prediction_by_area")
    ...
    # Start loop for each area
    for curArea in dict_of_regions_pred:
        # curArea = key

        log.info(f"... {curArea}")

        dfp = dict_of_regions_pred[curArea].copy()
        ...
        if list_ohmodels_catFtr_reload is None:
            ...
            log.warning(f"[{curArea}] No categorical models found")
            continue
        ...
        try:
            ...
            dfp, pred_period = run_prediction(dict_hp, dfp, list_ohmodels_catFtr_reload, bestModel_reload, log)
        except ValueError as e:
            # Propagate error :
            #   The feature names should match those that were passed during fit.
            # because this is a 'configuration' error and it is possible to correct
            # changing the configuration
            emsg = str(e)
            if "The feature names should match those that were passed during fit." in emsg:
                raise e
            log.error(f"[{curArea}] prediction failed:")
            exc = traceback.format_exc()
            log.error(f"... {e}\n{exc}")
    ...
    log.info("end [run_prediction_by_area]")
    return df_pred_out
