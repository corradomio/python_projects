...
def weighted_absolute_percentage_error(actuals, predictions):
        ...
        y_true = actuals.values.reshape(-1)
...
def createTrainTestSets(df, train_ratio, targetFeature, ignoreInputFeatures):
    ...
    test = df[train_size:]
    train_X = train[train.columns.difference(ignoreInputFeatures + [targetFeature])]
    train_Y = train[targetFeature]
    test_X = test[test.columns.difference(ignoreInputFeatures + [targetFeature])]
...
def test(model, test_X, test_Y):
        ...
        wape = weighted_absolute_percentage_error(test_Y, result)
...
def runModelsGetResults(list_regression_models, train_X, train_Y, test_X, test_Y, printLog):
        # WARN: printLog IS NOT a boolean value BUT a logging.Logger
        ...
        for key in list_regression_models:
            ...
            printLog.info(f"... ... {key}: wape={wape:.3}, r2={r2:.3}")
