...
from typing import Optional, Any, Iterator, Union, Iterable
...
class ProjectFiles:
    def __init__(self, programming_language: str = 'java'):
        ...
        self._log = Logger.getLogger("ProjectLoader")
    # end
    ...
    def files(self) -> list[Union[str, Path]]:
        return list(map(lambda fi: fi["file"], self._files))
    # end

    @property
    def names(self) -> list[str]:
        return list(map(lambda fi: fi["name"], self._files))
    ...
    def scan(self, home: str, rev: int = 0, refid: Optional[str] = None):
        """
        ...
        self._log.info(f"done [{len(self._files)}]")
    # end
