...
def xml_to_yolo():
    ...
    for f in LABELS.files("*.xml"):
        ...
        # end
        label_path = LABELS_YOLO / label_name
...
def main():
    ...
    # test__list = []
    #
    # for f in LABELS.files("*.jpg"):
    #     if random() < 0.2:
    #         test__list.append(f.stem)
    #     else:
    #         train_list.append(f.stem)
    #
    # train_list = pd.DataFrame(data=train_list, columns=["file"])
    # test__list = pd.DataFrame(data=test__list, columns=["file"])
    #
    # np.savetxt("yolo3_train.csv")
