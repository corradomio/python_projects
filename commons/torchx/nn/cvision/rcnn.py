...
class FasterRCNN(nn.Module):
    def __init__(
        ...
        model_config = dict(
            ...
            im_channels=in_channels,
            aspect_ratios=aspect_ratios,
            scales=scales,
            min_im_size=min_im_size,
            max_im_size=max_im_size,
            backbone_out_channels=backbone_out_channels,
            fc_inner_dim=fc_inner_dim,
            # ---
            rpn_bg_threshold=rpn_bg_threshold,
            rpn_fg_threshold=rpn_fg_threshold,
            rpn_nms_threshold=rpn_nms_threshold,
            rpn_train_prenms_topk=rpn_train_prenms_topk,
            rpn_test_prenms_topk=rpn_test_prenms_topk,
            rpn_train_topk=rpn_train_topk,
            rpn_test_topk=rpn_test_topk,
            rpn_batch_size=rpn_batch_size,
            rpn_pos_fraction=rpn_pos_fraction,
            # ---
            roi_iou_threshold=roi_iou_threshold,
            roi_low_bg_iou=roi_low_bg_iou,  # increase it to 0.1 for hard negative
            roi_pool_size=roi_pool_size,
            roi_nms_threshold=roi_nms_threshold,
            roi_topk_detections=roi_topk_detections,
    ...
    def forward(self, image_target):
        if self.training:
            ...
            image, target = image_target, None
