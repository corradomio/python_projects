...
def _clip_to_month_day(dt: datetime, last=False) -> datetime:
    if last:
        last_day = last_day_of(dt.year, dt.month)
        return dt.replace(microsecond=0, second=0, minute=0, hour=0, day=last_day)
    else:
...
CLIP_DATETIME = {
    ...
    "W": lambda dt: _clip_to_dow(dt, 0),        # Monday
    "WS": lambda dt: _clip_to_dow(dt, 0),       # Monday
    "WE": lambda dt: _clip_to_dow(dt, 6),       # Sunday
    ...
    "M": lambda dt: _clip_to_month_day(dt),         # year-month-1
    "MS": lambda dt: _clip_to_month_day(dt),        # year-month-1
    "ME": lambda dt: _clip_to_month_day(dt, True),  # year-month-last
...
def clip_date(dt: datetime, freq: str, right=False) -> datetime:
    """
    It clips the datetime to the correct frequency (daily, weekly, monthly):
    daily: at midnight (00:00:00)
    weekly: at monday
    monthly: the first day of the month
    If 'right' is False, 'clipped_date <= dt'
    If 'right' is True,   'clipped_date >= dt'
    """
    if freq in CLIP_DATETIME:
        clipped_date: datetime = CLIP_DATETIME[freq](dt)
    else:
        clipped_date = dt
    if right and dt > clipped_date:
        clipped_date = clipped_date + relativeperiods(periods=1, freq=freq)
    return clipped_date
