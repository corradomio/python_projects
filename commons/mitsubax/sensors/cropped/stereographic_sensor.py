"""
stereographic_sensor.py
------------------------
A stereographic wide-angle / fisheye camera plugin for Mitsuba 3.
...
Projection model: stereographic ("angle of view" conformal) fisheye.
    r = 2f * tan(theta / 2)
...
r = 1 at the edge of the configured field of view. Unlike the equidistant
(f-theta) mapping, the stereographic projection is *conformal* -- it
preserves local angles/shapes, so small objects away from the image
center don't get stretched the way they do under equidistant or
equisolid-angle fisheye models. It's the same projection used by "Little
Planet" panorama tools and many circular fisheye lens designs. It
supports FOVs up to (but excluding) 360 degrees.
...
-----
    ...
    mi.register_sensor('stereographic', lambda props: StereographicCamera(props))
...
assert mi.variant() is not None, "Variant must be set before importing mitsubax.stereographic_sensor"


class StereographicCamera(mi.Sensor):
    """Stereographic (conformal) fisheye sensor."""

    def __init__(self, props: mi.Properties):
        mi.Sensor.__init__(self, props)

        # Full field of view, in degrees (e.g. 180 = hemispherical fisheye).
        # Stereographic projection only blows up exactly at 360 degrees,
        ...
        #   r_norm(theta) = tan(theta / 2) / tan(theta_max / 2)
        self.m_theta_max = theta_max
        self.m_tan_half_theta_max = dr.tan(theta_max * 0.5)
    ...
    def _local_direction(self, film_p: mi.Point2f):
        ...
        # Inverse stereographic mapping: recover theta from the
        # normalized radius, theta = 2 * atan(r * tan(theta_max / 2)).
        theta = 2.0 * dr.atan(r_clamped * self.m_tan_half_theta_max)
...
# def register():
#     """Call once, after `mi.set_variant(...)`, to make 'stereographic'
...
#
#     img = mi.render(scene)
#     mi.util.write_bitmap('stereographic_test.png', img)
#     print('Rendered stereographic_test.png')
