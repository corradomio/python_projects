...
# ---------------------------------------------------------------------------
# GroupsStandardScaler
# ---------------------------------------------------------------------------

class GroupsStandardScaler(GroupsBaseEncoder):
    ...
    def __init__(self, columns=None,
        ...
        """
        Apply the scaler to the selected columns, normalizing the values
    ...
    def _compute_means_sdevs(self, X: PANDAS_TYPE):
    ...
    def _transform(self, X: PANDAS_TYPE, means, sdevs) -> PANDAS_TYPE:
        X = X.copy()
    ...
    def _scale(self, X: PANDAS_TYPE, means, sdevs):
        meanv = self._meanf
        sdevv = self._sdevf
...
# compatibility
GroupsStandardScalerEncoder = GroupsStandardScaler


# ---------------------------------------------------------------------------
# GroupsLinearMinMaxScaler
# ---------------------------------------------------------------------------

class GroupsLinearMinMaxScaler(GroupsBaseEncoder):
    ...
    def __init__(self, columns=None,
                 ...
                 *,
                 groups=None, copy=True):
        """
        Apply the scaler to the selected columns, in such way that
        min_value -> 0, max_value->1 (or the specified limits)
    ...
    def _compute_mins_maxs(self, X: PANDAS_TYPE):
        columns = self._get_columns(X)
        mins = {}
        maxs = {}
        for col in columns:
    ...
    def _transform(self, X: PANDAS_TYPE, mins, maxs) -> PANDAS_TYPE:
        X = X.copy()
    ...
    def _scale(self, X: PANDAS_TYPE, mins, maxs):
        minv = self._minf
        maxv = self._maxf
