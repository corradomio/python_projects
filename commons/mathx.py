...
def argsort(values: Iterable, descending: bool = False) -> Tuple[int]:
    """Sort the values in ascending (descending=True->descending) order and return the indices"""
    n = len(values)
...
def zero(x, eps=EPS) -> bool:
    """return 0 if the value is smaller than the epserance"""
    return 0. if -eps <= x <= +eps else x


def sign(x, zero=False, eps=EPS) -> int:
...
def isz(x: float, eps=EPS) -> bool:
    """is zero"""
    return abs(x) <= eps


def iseq(x: float, y: float, eps=EPS) -> bool:
    """is equal to"""
    return isz(x - y, eps=EPS)


def isgt(x: float, y: float, eps=EPS) -> bool:
    """is greater than"""
    return x > (y + eps)


def islt(x: float, y: float, eps=EPS) -> bool:
    """is less than"""
    return x < (y - eps)


def isge(x: float, y: float, eps=EPS) -> bool:
    """is greater or equal than"""
    return not islt(x, y, eps=EPS)


def isle(x: float, y: float, eps=EPS) -> bool:
