...
def assert_list(l, elem_type_: type, msg: Optional[str]=None):
    assert isinstance(l, (tuple, list)), msg
    for e in l:
        assert isinstance(e, elem_type_)


def assert_dict(d, key_type, value_type, msg: Optional[str]=None):
...
def assert_not_is_instance(obj, cls, msg=None):
    ...
    assert not isinstance(obj,lcs), msg
...
def assert_sequence_equal(seq1, seq2, msg=None, seq_type=None):
    ...
    n = len(seq1)
    for i in range(n):
        assert assert_equal(seq1[i], seq2[i])
...
def assert_tuple_equal(tuple1, tuple2, msg=None):
    ...
    assert_sequence_equal(list1, list2, msg, tuple)

def assert_set_equal(set1, set2, msg=None):
    """A set-specific equality assertion."""
    assert isinstance(seq1, set), msg
    assert isinstance(seq2, set), msg
...
def assert_dict_equal(d1, d2, msg=None):
    ...
    assert isinstance(d2, setdict), msg
