...
class _SixDRepNet360(nn.Module):
...
class SixDRepNet360(_SixDRepNet360):
    def __init__(self, gpu_id : int=0, dict_path: str=''):
        super().__init__(torchvision.models.resnet.Bottleneck, [3, 4, 6, 3], 6)
        ...
        if 'model_state_dict' in saved_state_dict:
            self.load_state_dict(saved_state_dict['model_state_dict'])
        else:
    ...
    def predict(self, image):
        if isinstance(image, np.ndarray):
            ftemp = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image = Image.fromarray(ftemp, "RGB")

        gpu = self._gpu
        # w, h, c = image.shape
        # images = image.reshape((1, w, h, c))
        # (1, 3, 224, 224)
        images = TRANSFORMATIONS(image)[None,:,:,:].cuda(gpu)

        R_pred = self(images).cpu()
        euler = compute_euler_angles_from_rotation_matrices(R_pred)

        # pitch, yaw, roll
        euler_deg = euler*180 / np.pi
        return euler_deg[:,0], euler_deg[:, 1], euler_deg[:, 2]
# end
