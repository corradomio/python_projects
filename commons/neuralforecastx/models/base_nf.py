...
def _to_nfdf(y: pd.Series, X: Optional[pd.DataFrame], ignores_exogenous_X) -> pd.DataFrame:
...
def _from_nfdf(predictions: list[pd.DataFrame], fha: ForecastingHorizon, y_template: pd.Series, model_name) -> pd.DataFrame:
...
def _to_pred_nfdf(fh: ForecastingHorizon, X: Optional[pd.DataFrame]):
    if X is None:
        return None
...
def _extends_nfdf(df: pd.DataFrame, y_pred: pd.DataFrame, X: Optional[pd.DataFrame], at: int, name: str):
    n_pred = len(y_pred)
...
def _concat_df(slist) -> pd.DataFrame:
    slist = [s for s in slist if s is not None]
    if len(slist) == 1:
        return slist[0]
    else:
...
class _BaseNFForecaster(ScaledForecaster):
    ...
    def _fit(self, y, X, fh):
        ...
        nfdf = _to_nfdf(y, X, self._ignores_exogenous_X)
    ...
    def _to_nfdf_ext(self, yext, X):
        if yext is None:
            return _to_nfdf(self._y, self._X, self._ignores_exogenous_X)

        n = len(yext)
        Xext = X.iloc[:n] if X is not None else None
        yall = _concat_ser([self._y, yext])
        Xall: Optional[pd.DataFrame] = _concat_df([self._X, Xext]) if self._X is not None else None
        nfdf = _to_nfdf(yall, Xall, self._ignores_exogenous_X)
        return nfdf
    ...
    def _predict_same(self, fh, X):
        nf_df = _to_nfdf(self._y, self._X, self._ignores_exogenous_X)
        ...
        model_name = _name_of(self._model)
        fha = fh.to_absolute(self._cutoff)
        return _from_nfdf([y_pred], fha, self._y, model_name)
    ...
    def _predict_recursive(self, fh, X):
        ...
        while l<n:
            ...
            h = len(y_nf)
            model_name = _name_of(self._model)
            y_pred = _from_nfdf([y_nf], fha[l:l + h], self._y, model_name)
            y_rec = _concat_ser([y_rec, y_pred])
            l = len(y_rec)
        # end
