...
def main():
    ...
    at = ppx.ArrayTransformer(xlags=[0, 1, 2], ylags=[0, 1, 2], tlags=[0, 1], temporal=False, swap=False)
    Xt_train0, yt_train0 = at.fit_transform(Xl_train, yl_train)
    Xt__test0, yt__test0 = at.transform(Xl__test, yl__test)

    at = ppx.ArrayTransformer(xlags=[0, 1, 2], ylags=[0, 1, 2], tlags=[0, 1], temporal=True, swap=False)
    Xt_train1, yt_train1 = at.fit_transform(Xl_train, yl_train)
    Xt__test1, yt__test1 = at.transform(Xl__test, yl__test)

    at = ppx.ArrayTransformer(xlags=[0, 1, 2], ylags=[0, 1, 2], tlags=[0, 1], temporal=True, swap=True)
