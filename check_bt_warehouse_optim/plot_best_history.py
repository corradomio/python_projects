...
def plot_history():
    for szdir in RESULTS_DIR.dirs():
        for jfile in szdir.files("*.json"):
            ...
            fdir: path = PLOTS_BEST_DIR / str(num_centers)
            fdir.makedirs_p()
            # fname = fdir / f"best-{num_centers}-{item_code}-{algo_name}-{ffname}.png"
            fname = fdir / "best-" + jfile.stem[3:] + ".png"
            ...
            best_fit_list = -np.array(best_fit_list)

            # TRICK!!!
            if best_fit_list.max() > 1.5:
                best_fit_list -= 1
