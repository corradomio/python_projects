...
def runPredictionForNewPeriod_new(cur_period, df, model, targetWeekLag, targetDayLag, inputsWeekLag, inputsDayLag, dowCol, targetFeature, inputFeaturesForAutoRegression, ignoreInputFeatures):
    # expand (in this case correct) autoregressive feature for any newly updated rows - but it currently does for all rows again 
    df = ar.expand_autoregressive_features(targetWeekLag, targetDayLag, inputsWeekLag, inputsDayLag, df, dowCol, targetFeature, inputFeaturesForAutoRegression)
    #print(df)
    ...
    df3_X = df3[df3.columns.difference(ignoreInputFeatures)]
...
def runPredictionForAllNewPeriods_new(df, model, targetWeekLag, targetDayLag, inputsWeekLag, inputsDayLag, dowCol, targetFeature, inputFeaturesForAutoRegression, ignoreInputFeatures):
    ...
    for i in range(pred_period):
        ...
        df = runPredictionForNewPeriod_new(cur_period, df, model, targetWeekLag, targetDayLag, inputsWeekLag, inputsDayLag, dowCol, targetFeature, inputFeaturesForAutoRegression, ignoreInputFeatures)
    return df
