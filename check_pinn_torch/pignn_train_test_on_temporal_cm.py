# -*- coding: utf-8 -*-
"""PIGNN_train_test_on_temporal_cm.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ApUxqJB2ZKg-XRiB9sUxPFEfMJrqhqMy
"""
...
plt.plot(x_node, velocity[0], "ro")
plt.title("node locations (with noise?)")
plt.show()

"""# Get adjacency matrix"""


def adjacency_matrix(velocity, x_node, window_size=5, alpha=0.5, threshold=0.4):
    """
    Enhanced adjacency matrix construction incorporating both spatial and temporal correlations

    Args:
        velocity: velocity data of shape [t, n_nodes]
    ...
    # Combine spatial ONLY correlation, NO temporal correlations
    A = Ws

    # find the threshold where the graph became connected
...
A, threshold = adjacency_matrix(velocity, x_node, threshold=0.7)
...
def get_A_hat(A):
    # A_tilde = A + I
    A_tilde = torch.tensor(A, dtype=torch.float32) + torch.eye(A.shape[0], dtype=torch.float32)

    # A_bin: A_bin[ij] = 1 if A_tilde[ij] != 0
    A_bin = A_tilde.clone()
    A_bin[A_bin != 0] = 1

    # computing D_tilde, D_tilde_inv and D_tilde_inv**(1/2)
    #
    # D_tilde = diag(sum_by_row(A_bin))
    # D_tilde_inv = inverse(D_tilde)
    # D_tilde_inv_sqrt = sqrt(D_tilde_inv)
    # replace 'nan' or 'inf' with 0
    # convert D_tilde_inv_sqrt in a torch.Tensor
...
def create_data(data, A, mask, set_size):
    ...
    for i in range(set_size):
        ...
        A_hat: torch.Tensor = get_A_hat(A_temp)

        # check IF A_hat is a connected graph
        G = nx.from_numpy_array(A_hat.numpy())
        if nx.is_connected(G):
            print(f"ERROR: A_hat[{i}] is a NOT CONNECTED GRAPH")
...
class GRCU(torch.nn.Module):
    def __init__(
        ...
        self.evolve_weights = mat_GRU_cell(self.in_feats, self.out_feats)

        self.activation = self.activation
        self.GCN_init_weights = Parameter(torch.Tensor(self.in_feats, self.out_feats))
    ...
    def forward(self, A_list, node_embs_list, mask_list):
        ...
        # print("GCN_weights", id(GCN_weights), GCN_weights.data_ptr())
...
class PredictionEGCN(nn.Module):
    def __init__(
        ...
        self.feats_per_node = feats_per_node
        self.layer_1_feats = layer_1_feats
        self.layer_2_feats = layer_2_feats
        self.layer_3_feats = layer_3_feats
        ...
        hidden_dim = self.layer_3_feats * self.n_nodes

        # WARNING!
        # MA HA SENSO usare len(time) (=500)
        # NON DOVREBBE ESSERE il numero di timestamp del train?
        #
        ...
        # nt = len(time)
        # self.node_specific_n2 = nn.Sequential(
        #     nn.Linear(nt, hidden_dim, 128, bias = False),
        #     nn.ReLU(),
        #     nn.Dropout(p_dropout),
        #     nn.Linear(nt, 128, 64, bias = False),
        #     nn.ReLU(),
        #     nn.Dropout(p_dropout),
        #     nn.Linear(nt, 64,30, bias = False)
        # )
