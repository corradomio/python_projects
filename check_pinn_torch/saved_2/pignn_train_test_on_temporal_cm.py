...
plt.plot(np.sort(x_node), velocity[0], "ro")
...
for epoch in range(n_epochs):
    ...
    if epoch % 2 == 0:
        print(f"Epoch {epoch}, Data Loss: {data_mean_loss.item():.6f}, physic_loss: {physics_loss.item()}, error on missing train: {missing_train}, error on available: {available_train}") #, Physic Loss: {loss_pde.item()}, total loss: {total_loss.item()}")
...
for epoch in range(n_epochs):
    ...
    if epoch % 2 == 0:
        print(f"Epoch {epoch}, Data Loss: {data_mean_loss.item():.6f}, physic_loss: {physics_loss.item()}, error on missing train: {missing_train}, error on available: {available_train}")
