...
def randomInitialize():
    for d, doc in enumerate(docs):
        zCurrentDoc = []
        for w in doc:
            pz = np.divide(np.multiply(theta[d, :], phi[:, w]), Nz)
...
def gibbsSampling():
    # D = len(docs)
    # V = len(word2id)
    # theta = np.zeros([D, T]) + alpha
    #   phi = np.zeros([T, V]) + beta
    #    Nz = np.zeros([T]) + V * beta
    ...
    for d, doc in enumerate(docs):
        for index, w in enumerate(doc):
            ...
            pz = np.divide(np.multiply(theta[d, :], phi[:, w]),        Nz)
            #                 (theta[d: ] + alpha), phi[:, w] + beta),(Nz + V*beta)
            # 按照计算出的分布进行采样
            z = np.random.multinomial(1, pz / pz.sum()).argmax()
            Z[d][index] = z
...
def perplexity():
    ...
    for d, doc in enumerate(docs):
        for w in doc:
            ll = ll + np.log(((phi[:, w] / Nz) * (theta[d, :] / nd[d])).sum())
            n = n + 1
    return np.exp(ll / (-n))
