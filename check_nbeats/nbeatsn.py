...
class NBeatsNet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(forecast_length, int):
            forecast_length = (forecast_length, 1)
        if isinstance(backcast_length, int):
            backcast_length = (backcast_length, 1)

        self.forecast_shape = (-1,) + tuple(forecast_length)
        self.backcast_shape = (-1,) + tuple(backcast_length)
        #
        self.forecast_length = _mul(forecast_length)
        self.backcast_length = _mul(backcast_length)
    ...
    def forward(self, backcast):
        ...
        backcast = torch.reshape(backcast, self.backcast_shape)
        forecast = torch.reshape(forecast, self.forecast_shape)
