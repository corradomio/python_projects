...
class BayesianOptimization(Observable):
    ...
    def maximize(self,
        ...
        _kappa = 2.576 if kappa is None else kappa
        _kappa_decay = 1 if kappa_decay is None else kappa_decay
        _kappa_decay_delay = 0 if kappa_decay_delay is None else kappa_decay_delay
        _xi = 0.0 if xi is None else xi
        ...
        if acquisition_function is None:
            util = UtilityFunction(kind='ucb',
                                   kappa=_kappa,
                                   xi=_xi,
                                   kappa_decay=_kappa_decay,
                                   kappa_decay_delay=_kappa_decay_delay)
        else:
