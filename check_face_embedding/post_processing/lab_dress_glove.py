...
# 7 = {str} 'white_glove'
#
DRESS_GLOVE_MODEL = YOLO(".yolo_weights/lab_dress_glove.pt").to(TORCH_DEVICE).eval()



def classify_image(image_file: str|Path) -> list[np.ndarray]:
    """
    ...
    """
    assert isinstance(image_file, str|Path), "Parameter 'image_file' must be a str"
    ...
    # The behaviour of this calss is more strange than I supposed!
    # boxes contains a list of 1,2,3 classes NON NECESSARLY all 3
    # then, we have
    #   boxes.cls   the class
    #   boxes.conf  the confidence
    # it is possible that some classifications are NOT AVAILABLE

    classes = boxes.cls.cpu().numpy()
    confs = boxes.conf.cpu().numpy()
    n_classes = len(boxes)
    # classes contains AT MAXIMUM 3 elements, NOT EXACTLY 3 elements!
    ...
    person_type = np.zeros(2, dtype=float)
    dress_type = np.zeros(2, dtype=float)
    glove_type = np.zeros(2, dtype=float)
    ...
    for i in range(n_classes):
        ...
        if c in [1,3,7]:
            ...
            person_type[0] = 1 - conf
            person_type[1] = conf
        else:
            print(f"ERROR: unknown class {c}")
    # end
