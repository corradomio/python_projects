...
def _get_predictor(cfg_name: str):
    global FASTREID_PREDICTORS
    if cfg_name in FASTREID_PREDICTORS:
        return FASTREID_PREDICTORS[cfg_name]

    cfg = _get_cfg(cfg_name)
    _set_weights_path(cfg_name, cfg)
    predictor = DefaultPredictor(cfg)

    FASTREID_PREDICTORS[cfg_name] = (cfg, predictor)
...
class FastReID:
    ...
    @staticmethod
    def represent(image: str | np.ndarray, model_name: str):
        cfg_name = model_name.replace("__", "/")
        cfg, predictor = _get_predictor(cfg_name)
